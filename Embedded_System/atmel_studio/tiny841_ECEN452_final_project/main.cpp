...
uint8_t SPI_comm(uint8_t TX_data);	//send data using SPI
...
volatile uint8_t data_out[4];	//send data	
volatile uint8_t byte_count = 0;	// count for 32 bytes of data
volatile bool first_comm = true;	
...
volatile uint8_t v_ref_H = 0;	//place as global only for debugging purpose
volatile uint8_t v_ref_L = 0;	//place as global only for debugging purpose
...
ISR(USART0_RX_vect, ISR_BLOCK){	//USART1 Receive complete interrupt service routine		
	...
	//data_in = UDR0;		//receive data from the USART1 receiver buffer; page 180		
	//if(data_in == 0xFF && first_comm == true){	//must be in Low, High order to use struct method in python	
		//UART_TX(v_ref_L);
		//UART_TX(v_ref_H);			
		//first_comm = false;
	//} else {
